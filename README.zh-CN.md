...
Demask ROI Lada 是一款适用于 Windows 的透明、全屏区域（ROI）马赛克修复
浮层工具。它会截取鼠标指针周围的方形区域，并在本地 NVIDIA GPU 上运行
Lada BasicVSR++ v1.2 时序修复模型。

软件基于屏幕捕获工作，不依赖浏览器 DOM 结构或网页播放器的具体实现。
...
如果这个项目对你有帮助，可以通过以下 EVM 兼容钱包地址支持项目继续开发：
